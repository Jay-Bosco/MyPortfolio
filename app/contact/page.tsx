  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await fetch("https://api.web3forms.com/submit", {
        ...
        // Web3Forms accepts JSON directly — no multipart needed
      });

      // Web3Forms requires Content-Type JSON, so resend properly
  ...
  return (
    <>
      ...
      <section className="relative pt-40 pb-20 overflow-hidden">
        <div className="absolute inset-0" style={{ backgroundImage: "url('/hero-bg.jpg')", backgroundSize: "cover", backgroundPosition: "center 70%" }} />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-12">
            ...
            <div className="lg:col-span-2 space-y-8">
              <div className="reveal glass rounded-2xl p-8 hover:border-blue-light/30 transition-all duration-400">
                <h3 className="font-display tracking-[2px] text-2xl mb-8">Contact <span className="text-blue-light">Info</span></h3>
                <ul className="space-y-6">
                  {contactDetails.map((d) => (
                    <li key={d.label} className="flex items-start gap-4 group">
                      <div className="w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0 mt-0.5 transition-all duration-300 group-hover:scale-110"
                      ...
                      <div>
                        ...
                        <a href={d.href} className="font-body text-white/65 hover:text-blue-light text-sm transition-colors duration-300">{d.value}</a>
                      </div>
                    </li>
                  ))}
                </ul>
                ...
                <div className="grid grid-cols-1 gap-3">
                  {socials.map((s) => (
                    <a key={s.href} href={s.href} target="_blank" rel="noopener noreferrer"
                      ...
                      <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0" style={{ background: "rgba(37,99,235,0.12)" }}>
                        <i className={`${s.icon} text-blue-light text-sm`} />
                      </div>
                      <span className="font-heading text-white/50 text-xs tracking-wide group-hover:text-white transition-colors duration-300">{s.label}</span>
                    </a>
                  ))}
                </div>
              </div>

              <div className="reveal reveal-delay-2 glass rounded-2xl p-6 border-blue-light/20 hover:border-blue-light/35 transition-all duration-400">
                <div className="flex items-center gap-3 mb-2">
                  <span className="w-2.5 h-2.5 rounded-full bg-emerald-400 animate-pulse" />
                  <span className="font-heading text-emerald-400 text-xs tracking-[3px] uppercase font-semibold">Available for Work</span>
                </div>
                <p className="font-body text-white/40 text-sm leading-relaxed">Currently open to freelance projects and full-time roles. Response within 24 hours.</p>
              </div>
            </div>
            ...
            <div className="lg:col-span-3 reveal reveal-delay-2">
              <div className="glass rounded-2xl p-10 hover:border-blue-light/25 transition-all duration-400">
                <h2 className="font-display tracking-[2px] text-3xl mb-2">Send a <span className="text-blue-light">Message</span></h2>
                <p className="font-body text-white/35 text-sm mb-8">Fill in the form below and I&apos;ll get back to you within 24 hours.</p>
                ...
                {status === "sent" ? (
                  <div className="flex flex-col items-center justify-center py-20 gap-5 text-center">
                    ...
                    <button onClick={() => setStatus("idle")} className="font-heading text-blue-light text-sm hover:text-white transition-colors duration-300 tracking-wide">
                  ...
                  <form onSubmit={handleSubmit} className="space-y-5">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                      {[{ name: "fname", placeholder: "First Name" }, { name: "lname", placeholder: "Last Name" }].map((f) => (
                        <input key={f.name} type="text" name={f.name} placeholder={f.placeholder} required
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                      {[{ name: "email", placeholder: "Email Address", type: "email" }, { name: "phone", placeholder: "Phone Number", type: "tel" }].map((f) => (
                    ...
                    <textarea name="message" placeholder="Tell me about your project or idea..." required rows={6}
                    ...
                    {status === "error" && (
                      <p className="font-body text-red-400 text-sm">
                        ❌ Something went wrong. Please email me directly at hamzatajibola401@gmail.com
                      </p>
                    )}
