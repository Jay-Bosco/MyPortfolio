...
export default function ProjectCard({ project, onImageClick }: Props) {
  ...
  return (
    <div className="group glass rounded-2xl overflow-hidden hover:border-blue-light/30 transition-all duration-500 hover:-translate-y-2 hover:shadow-[0_20px_50px_rgba(37,99,235,0.15)] flex flex-col">
      {/* Carousel */}
      <div className="relative h-52 overflow-hidden bg-ink-200 flex-shrink-0">
        <div
        ...
        {project.images.length > 1 && (
          <>
            <button onClick={prev} aria-label="Previous"
              className="absolute left-2 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-black/60 text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300 hover:bg-blue z-10 text-xs">
              <i className="fa-solid fa-chevron-left" />
            </button>
            <button onClick={next} aria-label="Next"
              className="absolute right-2 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-black/60 text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300 hover:bg-blue z-10 text-xs">
